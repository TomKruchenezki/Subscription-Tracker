...
@pytest.mark.parametrize("tier, pattern, amount, cycle, expected_score, expected_disposition", [
    # Tier 1 + receipt + amount → 0.60 + 0.30 + 0.10 = 1.00 (DETECTED)
    (1, PatternType.RECEIPT,       9.99, "UNKNOWN",  1.00, "DETECTED"),
    # Tier 1 + renewal → 0.60 + 0.25 = 0.85 (DETECTED)
    (1, PatternType.RENEWAL,       None, "UNKNOWN",  0.85, "DETECTED"),
    # Tier 1 + promotional → 0.60 - 0.30 = 0.30 (IGNORED)
    (1, PatternType.PROMOTIONAL,   None, "UNKNOWN",  0.30, "IGNORED"),
    # Tier 2 + receipt → 0.30 + 0.30 = 0.60 (FLAGGED)
    (2, PatternType.RECEIPT,       None, "UNKNOWN",  0.60, "FLAGGED"),
    # No match + receipt + amount → 0.00 + 0.30 + 0.10 = 0.40 (FLAGGED)
    (0, PatternType.RECEIPT,       9.99, "UNKNOWN",  0.40, "FLAGGED"),
    # No match + promotional → 0.00 - 0.30 = clamped 0.00 (IGNORED)
    (0, PatternType.PROMOTIONAL,   None, "UNKNOWN",  0.00, "IGNORED"),
    # Tier 1 only → 0.60 (FLAGGED)
    (1, PatternType.NONE,          None, "UNKNOWN",  0.60, "FLAGGED"),
])
